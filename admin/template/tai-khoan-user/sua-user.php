<?php 
	...
	function user ($id) {
		...
		if (isset($_POST['add_user'])) {
			...
			if ($ok == 'true') {
				$sql = "UPDATE user SET user_name = '$name', user_phone = '$phone', user_address = '$address', ngaysinh = '$birthday', city = $city, district = $district, village = $village WHERE user_id = $id";
				$result = mysqli_query($conn_vn, $sql);
				if ($result) {
					echo '<script type="text/javascript">alert(\'Bạn đã sửa được một khách hàng.\')</script>';
				} else {
	...
	if (empty($info['city'])) {
        ...
        $district = $action->getList('district', 'city_id', $info['city'], 'id', 'asc', '', '', '');
    }

    $village = array();
    if (!empty($info['district'])) {
        $village = $action->getList('village', 'district_id', $info['district'], 'id', 'asc', '', '', '');
    }
?>
...
<form action="" method="post" enctype="multipart/form-data">
    <div class="rowNodeContentPage">
        ...
        <div class="boxNodeContentPage">
            ...
            <input type="text" class="txtNCP1" name="name" value="<?= $info['user_name'] ?>" required/>
            <p class="titleRightNCP">Email</p>
            <input type="email" class="txtNCP1" name="email" value="<?= $info['user_email'] ?>" readonly/>
            <p class="titleRightNCP">Số điện thoại</p>
            <input type="tel" class="txtNCP1" name="phone" value="<?= $info['user_phone'] ?>" required/>
            <p class="titleRightNCP">Địa chỉ</p>
            <input type="text" class="txtNCP1" name="address" value="<?= $info['user_address'] ?>" required/>
            
            <p class="titleRightNCP">Ngày sinh</p>
            <input type="date" class="txtNCP1" name="birthday" value="<?= $info['ngaysinh'] ?>" required/>
            ...
            <select name="village" class="txtNCP1" id="village">
                ...
                <option value="<?= $item['id'] ?>" <?= $item['id']==$info['village'] ? 'selected' : '' ?> ><?= $item['name'] ?></option>
                <?php } ?>
            </select>
        </div>
