...

{
    ...
    $sql = "SELECT * From cart WHERE id_orderState = $state Order By id_cart DESC Limit $start,20";
...
if (isset($_GET['search']) && !empty($_GET['search'])) {
    ...
    echo '<div>'.$rows["paging"].'</div>';
        ...
        if (isset($_GET['trang'])) {
            ...
            $sql_pagin = "SELECT * From cart WHERE id_orderState = $state";
            ...
            $config = array(
                ...
                'link_full'     => 'index.php?page=don-hang-state&id='.$state.'&trang={trang}',// Link full có dạng như sau: domain/com/page/{page}

                'link_first'    => 'index.php?page=don-hang-state&id='.$state,// Link trang đầu tiên
            ...
            echo $pagination->htmlPaging($_GET['page']);
