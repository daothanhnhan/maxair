...
else
{
    ...
    // $rows = $order->getListOrder();
    $ngay_in = $_GET['date_in'];
    $ngay_out = $_GET['date_out'];
    $sql = "SELECT * From cart WHERE date_cart >= '$ngay_in' AND date_cart <= '$ngay_out' Order By id_cart DESC Limit $start,20";//echo $sql;
    ...
    ////////////////////////////////
    $sql = "SELECT * From cart WHERE date_cart >= '$ngay_in' AND date_cart <= '$ngay_out'";//echo $sql;
    $result = mysqli_query($conn_vn, $sql);
    $tong_tien = 0;
    while ($row = mysqli_fetch_assoc($result)) {
    ...
    <ul class="titleNP">
        ...
        foreach($rows['data'] as $rowOrder) :?>
            ...
            <li>
                ...
                <div class="colNP1_2">
                    <a href="index.php?page=sua-don-hang&id_cart=<?php echo $rowOrder['id_cart']; ?>" title="">#<?php echo $rowOrder['id_cart']; ?>
...
if (isset($_GET['search']) && !empty($_GET['search'])) {
    echo '<div>'.$rows["paging"].'</div>';
        ...
        if (isset($_GET['trang'])) {
            ...
            $sql_pagin = "SELECT * From cart WHERE date_cart >= '$ngay_in' AND date_cart <= '$ngay_out'";
            ...
            $config = array(
                ...
                'limit'         => 20,// limit
                'start'         => 0, // start
                'link_full'     => 'index.php?page='.$_GET['page'].'&date_in='.$_GET['date_in'].'&date_out='.$_GET['date_out'].'&trang={trang}',// Link full có dạng như sau: domain/com/page/{page}
                'link_first'    => 'index.php?page='.$_GET['page'].'&date_in='.$_GET['date_in'].'&date_out='.$_GET['date_out'],// Link trang đầu tiên
