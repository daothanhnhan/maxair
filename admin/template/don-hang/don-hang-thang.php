...
else
{
    ...
    // $rows = $order->getListOrder();
    $month = $_GET['month'];
    $sql = "SELECT * From cart WHERE date_cart like '%$month%' Order By id_cart DESC Limit $start,20";
    ...
    ////////////////////////////////
    $sql = "SELECT * From cart WHERE date_cart like '%$month%'";//echo $sql;
    $result = mysqli_query($conn_vn, $sql);
    $tong_tien = 0;
    while ($row = mysqli_fetch_assoc($result)) {
...
if (isset($_GET['search']) && !empty($_GET['search'])) {
    echo '<div>'.$rows["paging"].'</div>';
        ...
        if (isset($_GET['trang'])) {
            ...
            $sql_pagin = "SELECT * From cart WHERE date_cart like '%$month%'";
            ...
            $config = array(
                ...
                'limit'         => 20,// limit
                'start'         => 0, // start
                'link_full'     => 'index.php?page='.$_GET['page'].'&month='.$_GET['month'].'&trang={trang}',// Link full có dạng như sau: domain/com/page/{page}
                'link_first'    => 'index.php?page='.$_GET['page'].'&month='.$_GET['month'],// Link trang đầu tiên
