...
public function getListProductCat_byOrderASC(){
    ...
    $stateOrder_productCat='DESC';
    $rows = array();
    $sql = "SELECT * FROM $this->nameTable_productCat where $this->nameColStateProductCat_productCat = '1'  order by productcat_id $stateOrder_productCat";  
    $result = mysqli_query($conn_vn,$sql);
...
public function getProductList_byMultiLevel_orderProductId($idProductCat,$order_productId,$trang, $limit, $page){
    ...
    $list_id[] = $idProductCat;//var_dump($list_id);
...
public function num_selled ($product_id) {
    global $conn_vn;
    $sql = "SELECT * FROM cartdetail WHERE id_product = $product_id";
    $result = mysqli_query($conn_vn, $sql);
