...
class action_menu extends library{
    ...
    public function showMenu_byMultiLevel_mainMenuOnion($list_menu,$valParent_menu = 0,$lang, $level = 0){
        ...
        if ($cate_child) {    
            ...
            foreach ($cate_child as $key => $item) {
                ...
                if($level == 0 && $row[$this->nameColTitle_menuLanguage] == "Trang chủ")
                  echo '<li class="list_main_menu">'; 
                else
                    if($level == 0 && $row[$this->nameColTitle_menuLanguage] != "Trang chủ")
                        echo '<li class="item_main_menu">'; 
                    else    
                        echo '<li class="sub_main_menu1">'; 
                 echo '<a class="link_item_main_menu" href="'.$url.'">'.$row[$this->nameColTitle_menuLanguage].'</a>';
                    $this->showMenu_byMultiLevel_mainMenuOnion_SP($list_menu,$item[$this->nameColID_menu],$lang, $level+1);                
                echo '</li>';            
            }
            echo '</ul>';        
        } 
    ...
    /*-----------------------------*/
    public function showMenu_byMultiLevel_mainMenuOnion_SP($list_menu,$valParent_menu = 0,$lang, $level = 0){
        $cate_child = array();
        foreach ($list_menu as $key => $item)
        {
            // Nếu là chuyên mục con thì hiển thị
        ...
        if ($cate_child) {    
            ...
            foreach ($cate_child as $key => $item) {
                ...
                if($level == 0 && $row[$this->nameColTitle_menuLanguage] == "Trang chủ")
                  echo '<li class="item_sub_main_menu1">'; 
                else
                    if($level == 0 && $row[$this->nameColTitle_menuLanguage] != "Trang chủ")
                        echo '<li class="item_sub_main_menu1">'; 
                    else    
    ...
    /*-------------Menu Hat Giong Hoa Desktop----------------*/
    public function showMenu_byMultiLevel_mainMenuHatGiong($list_menu,$valParent_menu = 0,$lang, $level = 0){
        $cate_child = array();
        foreach ($list_menu as $key => $item)
        {
            // Nếu là chuyên mục con thì hiển thị
                ...
                if($level == 0 && $row[$this->nameColTitle_menuLanguage] == "Trang chủ")
                  echo '<li>'; 
                    ...
                    if($level == 0 && $row[$this->nameColTitle_menuLanguage] == "Sản phẩm")
                        ...
                        echo '<li>'; 
                 echo '<a href="'.$url.'" class="slide-section">'.$row[$this->nameColTitle_menuLanguage].'</a>';
                    $this->showMenu_byMultiLevel_mainMenuHatGiong($list_menu,$item[$this->nameColID_menu],$lang, $level+1);                
                echo '</li>';            
            }
            echo '</ul>';        
        } 
    ...
    /*-------------Menu Hat Giong Hoa Mobile----------------*/
    public function showMenu_byMultiLevel_mainMenuHatGiong_m($list_menu,$valParent_menu = 0,$lang, $level = 0){
        $cate_child = array();
        foreach ($list_menu as $key => $item)
        {
            // Nếu là chuyên mục con thì hiển thị
                ...
                if($level == 0 && $row[$this->nameColTitle_menuLanguage] == "Trang chủ")
                  echo '<li>'; 
                    ...
                    if($level == 0 && $row[$this->nameColTitle_menuLanguage] == "Sản phẩm")
                        ...
                        echo '<li>'; 
                 echo '<a href="'.$url.'">'.$row[$this->nameColTitle_menuLanguage].'</a>';
                    $this->showMenu_byMultiLevel_mainMenuHatGiong_m($list_menu,$item[$this->nameColID_menu],$lang, $level+1);                
                echo '</li>';            
            }
            echo '</ul>';        
        } 
    ...
    /////////////////////////  menu ////
    public function main_menu ($id, $lang) {
        global $conn_vn;
        $sql = "SELECT * From menu Inner Join menu_languages On menu.menu_id = menu_languages.menu_id Where menu_languages.languages_code = '$lang' And menu.state = 1 And menu.menu_parent = $id Order By menu.menu_sort_order ASC";
        $result = mysqli_query($conn_vn, $sql);
        $rows = array();
    ...
    public function show_menu1 ($list, $lang) {
        // $menu1 = new action_menu();
        echo '<ul class="list_main_menu_1">';
        foreach ($list as $item) {
            $url1 = $this->setUrlFriendly_byType($item['menu_id'], $lang);
            echo '<li class="item_main_menu_1">';
            if (strpos($url1, 'http') != false) {
                $url1 = substr($url1, 1);
            }
            echo '<a href="'.$url1.'" class="link_main_menu_1">'.$item['lang_menu_name'].'</a>';
            // echo '<a href="http://alibabatinhgia.webmienphi.org.vn" class="link_main_menu_1">'.$item['lang_menu_name'].'2</a>';
            $list_sub = $this->main_menu($item['menu_id'], $lang);
            if (count($list_sub) != 0) {
                $this->show_menu2($list_sub, $lang);
            }
            echo '</li>';
        }
        echo '</ul>';
    }
    public function show_menu11 ($list, $lang) {
        // $menu1 = new action_menu();
        echo '<ul class="list_main_menu_1">';
        $d=0;
        foreach ($list as $item) {
            $d++;
            $url1 = $this->setUrlFriendly_byType($item['menu_id'], $lang);
            echo '<li class="item_main_menu_1" id="demo'.$d.'">';
            if (strpos($url1, 'http') != false) {
                $url1 = substr($url1, 1);
            }
            $active = '';
            ...
            echo '<a href="'.$url1.'" class="link_main_menu_2 '.$active.'">'.$item['lang_menu_name'].'</a>';
            // echo '<a href="http://alibabatinhgia.webmienphi.org.vn" class="link_main_menu_1">'.$item['lang_menu_name'].'2</a>';
            $list_sub = $this->main_menu($item['menu_id'], $lang);
            if (count($list_sub) != 0) {
                $this->show_menu2($list_sub, $lang);
    ...
    public function show_menu2 ($list, $lang) {
        // $menu2 = new action_menu();
        echo '<ul class="list_main_menu_2">';
        foreach ($list as $item) {
            $url2 = $this->setUrlFriendly_byType($item['menu_id'], $lang);
            echo '<li class="item_main_menu_2">';
    ...
    public function show_menu3 ($list, $lang) {
        // $menu1 = new action_menu();
        // echo '<ul class="list_main_menu_1">';
        foreach ($list as $item) {
            $url1 = $this->setUrlFriendly_byType($item['menu_id'], $lang);
